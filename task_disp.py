...
class task_disp(tk.Frame):
    ...
    def ask_percent_completion(self):
        ...
        oldcumsum = int(self.cumcompleted.get())
        

        e = tk.Entry(t2, width=10, textvariable = self.cumcompleted)
        e.grid(row=0, column=1, padx=2, pady=2, sticky='we')
        e.focus_set()
        tk.Button(t2, text="Confirm", underline=0,  command=lambda: self.on_pause_completion(oldcumsum,t2) ).grid(row=0, column=2, sticky='e'+'w', padx=2, pady=2)
        
    def on_pause_completion(self, oldcumsum, t2):
        self.deltacompleted = int(self.cumcompleted.get()) - int(oldcumsum)
        #print self.deltacompleted
        t2.destroy()
